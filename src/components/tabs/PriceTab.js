...
function PriceTab({ data }) {
  const gap = data.kpi.retailPrice.value - data.kpi.rawMilkPrice.value;
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '14px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        <KpiCard title="가격 갭" value={gap.toLocaleString()} unit="원" yoy={1.4} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
        ...
        />
        <DairyChart
          title="원유 수취가격 추세 (원/kg)"
          data={data.priceTrend}
          lines={[{ key: 'rawMilk', name: '수취가격', color: '#f59e0b' }]}
        />
      </div>
    </div>
  );
